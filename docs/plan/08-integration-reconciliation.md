...
### 7.7.5 W3D3-α 실행 결과 — option α (ASM rewriting + R.jar seeding) CLOSED via Branch (B) (2026-04-29 α 후속)

**완료 (push)**:
- Option α 채택 (사용자 결정). 4 main + 7 test = 11 신규 파일 + 8 수정.
- `RewriteRules` (NAME_MAP 4 entries — Build family only, dual-publish 21개는 의도된 layoutlib SHIM 으로 무수정), `AndroidClassRewriter` (ASM ClassRemapper), `RJarTypeMapping` (R$type → ResourceType), `RJarSymbolSeeder` (R.jar walker, R$styleable 전체 skip).
- `AarExtractor` 가 classes.jar 를 stream-rewrite (REWRITE_VERSION cache layer).
- `MinimalLayoutlibCallback` 의 initializer 추가 (init 에서 R.jar seed). `FIRST_ID = 0x7F80_0000` (AAPT type-byte 와 disjoint).
- `LayoutlibRenderer.seedRJarSymbols` named method (CLAUDE.md "Lambdas — Avoid").
- `activity_basic_minimal.xml` contingency (Material 회피 layout, ConstraintLayout 유지).
- `LayoutlibRendererIntegrationTest` enable + `renderWithMaterialFallback` (renderer.lastSessionResult.exception/errorMessage 검사).
- 테스트: **167 unit PASS** (W3D3 base 142 → +25). **integration: 12 PASS + 1 SKIP** (tier3-glyph 만). LayoutlibRendererIntegrationTest 가 더 이상 SKIP 아님.

**T1 gate empirical 결과 — Branch (B)**:
primary `activity_basic.xml` (MaterialButton 포함) 시도 → `ERROR_INFLATION msg="The style on this component requires your app theme to be Theme.AppCompat (or a descendant)" exc=IllegalArgumentException`. fallback `activity_basic_minimal.xml` (standard Button) → SUCCESS. **Codex round 1 B2 hypothesis (Material theme enforcement throw) confirmed**.

**페어 리뷰**:
- **Spec round 1**: Claude NO_GO 2 hard blockers (NAME_MAP overrewrite 21 dual-publish, R$styleable index pollution) → spec round 2 반영. Codex stalled-final (long-tool-trace 후 verdict 미생성). Single-source verdict (Claude empirical) 진행.
- **Plan round 2**: Claude NO_GO 1 hard blocker (fallback retry semantics — stack trace 만 보면 Material frame 부재). Codex NO_GO 2 hard blockers (동일 + parseRClassName private 테스트). Direction 컨버전스. Plan v2 가 양측 모든 fix 반영.

**carry list** (W3D4+ 갱신):
- **MATERIAL-FIDELITY** (신규 추천): app/library resource value loader 추가 → sample-app `Theme.AxpFixture` (Material3.DayNight) 를 RenderResources 에 주입 → primary `activity_basic.xml` (MaterialButton 포함) contingency 없이 PASS. W3D1 framework loader 와 동일 패턴, ~500-1000 LOC.
- 기존: `tier3-glyph` (W4+), `CLI-DIST-PATH` (W3D2 close), `TEST-INFRA-SHARED-RENDERER` (W3D2 close), `POST-W2D6-POM-RESOLVE`, `W3-CLASSLOADER-AUDIT` (본 α 가 partial — Build shim 부재 + R.jar id seeding 처리, app resource values 는 carry).
